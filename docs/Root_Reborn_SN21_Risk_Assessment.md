...
**Assessment status:** Living document. Models run on **illustrative inputs** — see §3.0 before citing any figure.
...
It is a *managed exposure*, not a gift. It introduces one genuinely new tail risk (R-C) that compounds badly with Conviction's lockups, and one structural bias against our profile (R-J). The headline modelled result: **a favoured SN21 that accumulates ~5–6 weeks of basket weight, then suffers a full redemption, can breach the dereg-relevant floor in a single cascade.** *Now re-run on live pool state (§3, §6): with spot +15.8% above the 0.0035 floor, the breach threshold is ~41 days at 1% weight share — so the 30-day case lands ~3.8% above the floor and "5–6 weeks" sits at the edge of the breach band, not inside it. Direction holds; the trigger is slightly further out than v0.4 implied.*
...
## 6. Live SN21 values (connector pull, block 8,432,852 · 2026-06-18)

Pulled via the project's finney path (`subtensor.all_subnets()` + root metagraph), the same connectors that feed `market_sync.py` / `weights_scan.py`. TAO-denominated to strip TAO's own market move.

**Pool state (feeds §3 / R-C, R-F):**
| Metric | Value |
|--------|-------|
| alpha reserve (`alpha_in`) | 1,974,778 |
| TAO reserve (`tao_in`) | 8,002 TAO |
| spot price | 0.0040520 TAO/alpha |
| `alpha_out` (circulating) | 3,307,553 |
| 24h subnet volume | 519,793 |

**Liquidity & relative position (feeds R-J / Q5 / R-A / R-F):**
- **TAO depth rank: 69 / 128** priced subnets (~54th pct) — median-ish, **~1.05× field median (7,585 TAO)**, and **~4× below the top-liquidity decile** (≥33,910 TAO). → R-J: we are *underweighted-despite-demand* profile, not a deep safe-bet (Q5/D3).
- **Price rank: 35 / 128** (~27th pct) — SN21 alpha is cheaper than ~73% of the field; median subnet price 0.005768 vs our 0.004052. → R-A: below-median *position*, room for relative erosion.
- **Floor proximity (R-F):** spot is **+15.8% above** the 0.0035 dereg-relevant floor; **17 / 128** priced subnets currently sit below 0.0035. Not near-floor today, but not comfortably clear either.

**Root-stake concentration (feeds R-E / Q2):**
- Root subnet (netuid 0): **64 neurons, ~5,471,380 TAO** total stake.
- top-1 **18.5%**, top-3 **44.7%**, **top-4 55.3%**, top-5 60.3%, **top-10 80.4%**.
- → R-E temper: the live *root-stake* top-4 is **~55%, not the ~91%** the register cites (the 91% figure is a different/derived metric). Concentration is real and "court 4–5 entities" holds (top-5 = 60%), but on the raw root-stake metric the oligopoly is somewhat less extreme than stated. **Q2 remains off-chain** — chain shows *who is large*, not *who is hostile*.
